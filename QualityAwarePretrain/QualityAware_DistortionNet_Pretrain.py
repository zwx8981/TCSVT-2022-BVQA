...
class DBCNN(torch.nn.Module):
    ...
    def __init__(self, options):
        ...
        if options['fc'] == True:
            ...
            nn.init.kaiming_normal_(self.fc_D.weight.data)
            if self.fc_D.bias is not None:
                nn.init.constant_(self.fc_D.bias.data, val=0)
            nn.init.kaiming_normal_(self.fc_DL.weight.data)
            if self.fc_DL.bias is not None:
                nn.init.constant_(self.fc_DL.bias.data, val=0)
...
class DBCNNManager(object):
    def __init__(self, options, path):
        ...
        if self._options['fc'] == True:
            self._solver = torch.optim.SGD(
                [{'params': self._net.module.fc_D.parameters(), 'lr': self._options['base_lr']},
                 {'params': self._net.module.fc_DL.parameters(), 'lr': self._options['base_lr']}],
        ...
        if self._options['dataset'] == 'kadis700k':
        ...
        if self._options['dataset'] == 'kadis700k':
            import Kadis700kFolder_DistortionNet
            train_data = Kadis700kFolder_DistortionNet.Kadis700kFolder_DistortionNet(
                root=self._path['kadis700k'], loader=default_loader, index=self._options['train_index'],
                transform=train_transforms)
            test_data = Kadis700kFolder_DistortionNet.Kadis700kFolder_DistortionNet(
                root=self._path['kadis700k'], loader=default_loader, index=self._options['test_index'],
                transform=test_transforms)
        else:
            raise AttributeError('Only support KADIS-700k right now!')
    ...
    def train(self):
        ...
        best_plcc = 0.0
        best_acc = 0.0
        print('Epoch\tLr\tTotal loss\tCross loss\tHinge loss\tTrain_ACC\tTest_ACC\tTrain_SRCC\tTest_SRCC\tTest_PLCC')
        for t in range(self._options['epochs']):
            ...
            for sample in self._train_loader:
                ...
                predict_D, predict_DL = self._net(I)
                pscores = pscores + predict_DL.cpu().tolist()
            ...
            print('%d\t\t%4.10f\t\t%4.3f\t\t%4.3f\t\t%4.3f\t\t%4.4f\t\t%4.4f\t\t%4.4f\t\t%4.4f\t\t%4.4f' %
                  (t + 1, self._solver.param_groups[0]['lr'], sum(epoch_loss) / len(epoch_loss), sum(cross_loss) / len(cross_loss),
                   sum(hinge_loss) / len(hinge_loss), train_acc, test_acc, train_srcc, test_srcc, test_plcc))
    ...
    def _consitency(self, data_loader):
        ...
        for sample in data_loader:
            ...
            predict_D, predict_DL = self._net(I)
            pscores = pscores + predict_DL[:, 0].cpu().tolist()
            tscores = tscores + y_M.cpu().tolist()
