...
class CNNModel(torch.nn.Module):
    ...
    def __init__(self, model='ResNet-50'):
        ...
        if model == 'SpatialExtractor':
            print("use SpatialExtractor_QualityAwareDistortionNet")

            from QualityAwarePretrain.QualityAware_DistortionNet_Finetune import DBCNN
            options = {'fc': True}
            model = nn.DataParallel(DBCNN(options), device_ids=[0]).cuda()

            ckpt = './db_models/net_params_best.pkl'  # your path
            checkpoint = torch.load(ckpt)
            model.load_state_dict(checkpoint)

            self.features = nn.Sequential(*list(model.module.features1.children())[:-2])
        else:
