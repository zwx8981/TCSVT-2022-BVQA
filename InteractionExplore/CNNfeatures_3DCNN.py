...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        video_depth = 3
        video_batch = video_length // video_depth
        transformed_video = torch.zeros([video_batch, video_channel, video_depth, video_height, video_width])
        frame = torch.zeros([video_depth, video_channel, video_height, video_width])
        for batch_idx in range(video_batch):
            for i in range(video_depth):
                ff = video_data[batch_idx * video_depth + i]
                ff = Image.fromarray(ff)
                ff = transform(ff)
                frame[i] = ff
            transformed_video[batch_idx] = frame.permute(1, 0, 2, 3)
...
class CNNModel(torch.nn.Module):
    ...
    def __init__(self, model='ResNet-50'):
        ...
        if model == 'R3D':
            print("use R3D")
            self.features = nn.Sequential(*list(models.video.r3d_18(pretrained=True).children())[:-2])
        else:
