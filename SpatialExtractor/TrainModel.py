...
class Trainer(object):
    def __init__(self, config):
        ...
        self.spaq_data = ImageDataset(csv_file=os.path.join(config.spaq_set, 'splits2', str(config.split), 'spaq_test.txt'),
        ...
        self.train_loss = []
        self.train_std_loss = []
        self.test_results_srcc = {'bid': [], 'clive': [], 'koniq10k': [], 'spaq': []}
        self.test_results_plcc = {'bid': [], 'clive': [], 'koniq10k': [], 'spaq': []}
    ...
    def _train_single_epoch(self, epoch):
        ...
        if (not self.config.fc) & ((epoch+1) % self.epochs_per_eval == 0):
            ...
            self.test_results_srcc['spaq'].append(test_results_srcc['spaq'])
            ...
            self.test_results_plcc['spaq'].append(test_results_plcc['spaq'])

            out_str = 'Testing: BID SRCC: {:.4f} CLIVE SRCC: {:.4f}  KONIQ10K SRCC: {:.4f} ' \
                      'SPAQ SRCC: {:.4f}'.format(
                test_results_srcc['bid'],
                test_results_srcc['clive'],
                test_results_srcc['koniq10k'],
                test_results_srcc['spaq'])
            out_str2 = 'Testing: BID PLCC: {:.4f} CLIVE PLCC: {:.4f}  KONIQ10K PLCC: {:.4f} ' \
                       'SPAQ PLCC: {:.4f}'.format(
                test_results_plcc['bid'],
                test_results_plcc['clive'],
                test_results_plcc['koniq10k'],
                test_results_plcc['spaq'])
    ...
    def eval(self):
        ...
        if self.config.eval_spaq:
            q_mos = []
            q_hat = []
            for step, sample_batched in enumerate(self.spaq_loader, 0):
                x, y = sample_batched['I'], sample_batched['mos']
                x = Variable(x)
                x = x.to(self.device)
            ...
            srcc['spaq'] = scipy.stats.mstats.spearmanr(x=q_mos, y=q_hat)[0]
            plcc['spaq'] = scipy.stats.mstats.pearsonr(x=q_mos, y=q_hat)[0]
        else:
            srcc['spaq'] = 0
            plcc['spaq'] = 0
